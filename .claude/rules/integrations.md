...
- Auth: OIDC via `openid-client` (v6) + `@oslojs/crypto` / `@oslojs/encoding` (the `oslo` umbrella is
  deprecated), server-side Postgres sessions. BankID/Vipps through the Criipto/Signicat broker is app
  login; **ID-porten is separate**, scoped to Altinn filing only.
