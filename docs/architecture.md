...
- **The one hard boundary — `@saldo/domain` (pure):** 58 source modules, relative-imports-only, no wall-clock/random (enforced by `pnpm arch:check`). Submodules: banking, catalog, extraction, honest-number, ids, invoice, money, mva-melding, peppol, posting, reconciliation, reporting, rules, saft, tax, time, vat, xml.
- **`apps/web` modules (impure side):** auth, components, contracts, copy, db, documents, integrations, jobs, lib, observability, routes.
- **Client↔server boundary:** 46 route/index declarations in [`apps/web/app/routes.ts`](../apps/web/app/routes.ts) — loaders/actions, no separate API.
- **SQL integrity surface** (20 migrations in `db/migrations/*.sql`): 22 tables, 25 RLS policies, 10 triggers, 19 functions. Integrity triggers: bank_transaction_append_only, invoice_immutable, invoice_line_immutable, posting_immutable, posting_immutable_insert, posting_period_lock, supplier_invoice_immutable, supplier_invoice_line_immutable, voucher_immutable, voucher_period_lock.
<!-- /AUTOGEN:arch-graph -->
