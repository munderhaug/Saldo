...
- **The one hard boundary — `@saldo/domain` (pure):** 43 source modules, relative-imports-only, no wall-clock/random (enforced by `pnpm arch:check`). Submodules: banking, catalog, extraction, honest-number, ids, invoice, money, mva-melding, peppol, posting, reconciliation, reporting, rules, saft, tax, time, vat.
- **`apps/web` modules (impure side):** auth, components, contracts, copy, db, documents, integrations, jobs, lib, observability, routes.
- **Client↔server boundary:** 34 route/index declarations in [`apps/web/app/routes.ts`](../apps/web/app/routes.ts) — loaders/actions, no separate API.
