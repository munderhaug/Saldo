 ...
 * letters "k r", or, with no unit at all, a bare number with no indication it is money. Mirrors the
 * `Amount` helper on the honest-number reveal. Format comes from the domain helper (`formatKr`), never
 * raw `toFixed`.
 */
...
export function Money({ ore }: { ore: number }) {
  return (
    <>
      {formatKr(øre(ore))}&nbsp;<span aria-hidden="true">{t('common.currency')}</span>
