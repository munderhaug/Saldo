...
export function meta() {
  return [{ title: t('owner.new.title') }];
}
...
export async function action({ request, params }: Route.ActionArgs) {
  ...
  const parsed = ownerEventInput.safeParse({
    kind: form.get('kind'),
    amount: form.get('amount'),
  });
  if (!parsed.success) return { error: t('owner.new.errorInvalidInput') };
...
export default function NewOwnerEvent({ loaderData, actionData }: Route.ComponentProps) {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<OwnerEventInput>({
    resolver: zodResolver(ownerEventInput),
    mode: 'onTouched',
    defaultValues: { kind: 'drawing', amount: '' },
  });
  ...
  return (
    <main className="mx-auto grid max-w-xl gap-6 p-6 sm:p-10">
      <header className="grid gap-1">
        ...
        <h1 className="font-text text-2xl tracking-tight">{t('owner.new.title')}</h1>
        <p className="text-muted-foreground">{t('owner.new.intro')}</p>
      </header>
      ...
      >
        <fieldset className="grid gap-3">
          <legend className="font-text text-sm">{t('owner.new.kindLegend')}</legend>
          {OWNER_EVENT_KINDS.map((kind) => (
            <label key={kind} className="flex items-start gap-3">
              ...
              <span className="grid gap-0.5">
                <span className="font-text text-sm">{t(`owner.new.kind.${kind}.label`)}</span>
                <span id={`kind-${kind}-desc`} className="text-muted-foreground text-sm">
                  {t(`owner.new.kind.${kind}.desc`)}
                </span>
              </span>
            </label>
          ))}
        ...
        <p className="text-muted-foreground text-sm">{t('owner.new.confirmNote')}</p>
